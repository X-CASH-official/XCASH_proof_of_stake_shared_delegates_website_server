...
int send_data(const int SOCKET, char* data)
{   
...
/*
-----------------------------------------------------------------------------------------------------------
Name: get_delegate_voting_information
Description: Gets all of the public addresses and their total amount for the delegate
Parameters:
  SOCKET - The socket
  message - Where the data is stored
  STRING - The end string to know the socket is done receving data
...
int get_delegate_voting_information(const int SOCKET, char *message, const char* STRING, const int RECEIVE_DATA_SOCKET_TIMEOUT_SETTINGS, const int RECEIVE_DATA_SOCKET_TIMEOUT)
{
  // Variables
  int count = 0;
  char buffer [BUFFER_SIZE];
